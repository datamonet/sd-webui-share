...
db_file = os.path.join(scripts.basedir(), "wshare.sqlite3")
np = "Negative prompt: "
st = "Steps: "
timeout = 30
...
def check():
    if not os.path.exists(db_file):
        ...
        print("wshare: Creating database")
        create_db(cursor)
    ...
    if db_version[0] <= "2":
        ...
        print(f"wshare: Upgrading database from version {db_version[0]} to version {version}")
    ...
    if db_version[0] <= "5":
        ...
        update_db_data(cursor, "version", version)
        print(f"wshare: Database upgraded from version {db_version[0]} to version {version}")
    transaction_end(conn, cursor)
